...
function Products({ token }) {
  ...
  const handleDelete = async (id) => {
    ...
    await api.delete(`/api/products/${id}`, authHeader(token)); load();
  };
        ...
        <form onSubmit={handleSubmit} className="product-form">
          ...
          <div className="form-group">
            ...
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
              ...
              {['Accessories', 'Home', 'Electronics', 'Stationery', 'Wellness'].map((c) => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div className="form-group">
...
function Orders({ token }) {
  ...
  const updateStatus = async (id, status) => {
    await api.patch(`/api/admin/orders/${id}/status`, { status }, authHeader(token)); load();
  };
  return (
    <div>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o._id}>
                  ...
                  <td><p>{o.user?.name || 'N/A'}</p><p className="text-muted">{o.user?.email}</p></td>
                  <td>{o.items.length} item(s)</td>
                  <td>NPR {o.total?.toLocaleString()}</td>
                  <td>
                    <select className={`status-select status-${o.status}`} value={o.status} onChange={(e) => updateStatus(o._id, e.target.value)}>
...
function Users({ token }) {
  ...
  useEffect(() => { load(); }, []);
  const updateRole = async (id, role) => { await api.patch(`/api/admin/users/${id}/role`, { role }, authHeader(token)); load(); };
  const deleteUser = async (id) => { if (!window.confirm('Delete this user?')) return; await api.delete(`/api/admin/users/${id}`, authHeader(token)); load(); };
  return (
    <div>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u._id}>
                  ...
                  <td>
                    <select className={`role-select role-${u.role}`} value={u.role} onChange={(e) => updateRole(u._id, e.target.value)}>
                  ...
                  <td><button className="action-btn delete" onClick={() => deleteUser(u._id)}>Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Messages({ token }) {
  ...
  useEffect(() => { api.get('/api/admin/messages', authHeader(token)).then(({ data }) => setMessages(data)); }, []);
  return (
    <div>
      ...
      <div className="admin-card">
        {messages.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>No messages yet.</p> : (
          <div className="table-wrap">
            <table className="admin-table">
              <thead><tr><th>From</th><th>Email</th><th>Product</th><th>Message</th><th>Date</th></tr></thead>
              <tbody>
                {messages.map((m) => (
                  <tr key={m._id}>
